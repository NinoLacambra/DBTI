...
test("Interact with frames", async ({ page }) => {
    ...
    await frame.locator("input[name='fname']").fill("Nino");
    await frame.locator("input[name='lname']").fill("Lacambra");

    const innerFrame = frame.frameLocator("iframe[src='innerFrame']")
    await innerFrame.locator("input[name='email']").fill("lacambranino12@gmail.com")
